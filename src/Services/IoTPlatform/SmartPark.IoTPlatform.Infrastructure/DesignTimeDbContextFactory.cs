...
public sealed class IoTPlatformDesignTimeDbContextFactory : IDesignTimeDbContextFactory<IoTPlatformDbContext>
{
    public IoTPlatformDbContext CreateDbContext(string[] args)
    {
        var optionsBuilder = new DbContextOptionsBuilder<IoTPlatformDbContext>();
        optionsBuilder.UseNpgsql(ResolveConnectionString(args));
        return new IoTPlatformDbContext(optionsBuilder.Options, new SystemDateTimeProvider(), DesignTimeCurrentUser.Instance);
    }

    private static string ResolveConnectionString(string[] args)
    {
        const string defaultConnectionString = "Host=localhost;Port=5432;Database=smartpark_iot;Username=postgres;Password=postgres";
