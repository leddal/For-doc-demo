...
public sealed class WorkOrderDesignTimeDbContextFactory : IDesignTimeDbContextFactory<WorkOrderDbContext>
{
    public WorkOrderDbContext CreateDbContext(string[] args)
    {
        ...
        return new WorkOrderDbContext(optionsBuilder.Options, new SystemDateTimeProvider(), DesignTimeCurrentUser.Instance);
    }

    private static string ResolveConnectionString(string[] args)
    {
        const string defaultConnectionString = "Host=localhost;Port=5432;Database=smartpark_workorder;Username=postgres;Password=postgres";
