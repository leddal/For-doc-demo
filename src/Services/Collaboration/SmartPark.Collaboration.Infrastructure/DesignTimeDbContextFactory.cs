...
public sealed class CollaborationDesignTimeDbContextFactory : IDesignTimeDbContextFactory<CollaborationDbContext>
{
    public CollaborationDbContext CreateDbContext(string[] args)
    {
        ...
        return new CollaborationDbContext(optionsBuilder.Options, new SystemDateTimeProvider(), DesignTimeCurrentUser.Instance);
    }

    private static string ResolveConnectionString(string[] args)
    {
        const string defaultConnectionString = "Host=localhost;Port=5432;Database=smartpark_collaboration;Username=postgres;Password=postgres";
