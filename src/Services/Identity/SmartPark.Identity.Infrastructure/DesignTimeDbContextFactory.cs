...
public sealed class IdentityDesignTimeDbContextFactory : IDesignTimeDbContextFactory<IdentityDbContext>
{
    public IdentityDbContext CreateDbContext(string[] args)
    {
        ...
        return new IdentityDbContext(optionsBuilder.Options, new SystemDateTimeProvider(), DesignTimeCurrentUser.Instance);
    }

    private static string ResolveConnectionString(string[] args)
    {
        const string defaultConnectionString = "Host=localhost;Port=5432;Database=smartpark_identity;Username=postgres;Password=postgres";
